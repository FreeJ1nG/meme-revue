...
export default function SearchAndSavedBar({ search, handleClick }) {
  return (
    <div className="flex justify-center items-center">
      <div className="relative h-12 rounded-xl bg-black/10 flex justify-center xl:w-1/3 lg:w-2/5 md:w-1/2 sm:w-3/4">
        <div className="absolute flex top-0 bottom-0 left-0 right-0">
          <div className={(search ? "w-0" : "w-1/2") + " duration-500 transition-all"}></div>
          <div className="h-12 w-1/2 bg-gradient-to-t from-third to-second border-4 border-white/10 rounded-xl"></div>
        </div>
        <button
          className="group absolute left-0 w-1/2 py-3 px-16"
        ...
        <button
          className="group absolute right-0 w-1/2 py-3 px-16"
